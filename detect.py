...
from utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, check_suffix, colorstr,\
    scale_coords, xyxy2xywh, strip_optimizer, set_logging, increment_path, print_args, save_one_box
...
class simulation_opt:  # 参数对象。
    ...
    def __init__(self, weights, img_size=640, conf_thres=0.60, iou_thres=0.45, device='', view_img=False,
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    plot_one_box(xyxy, im0, label=label,
                                 color=self.colors[int(cls)], line_thickness=5)
            # result.append((im0, result_txt))  # 对于每张图片，返回画完框的图片，以及该图片的标签列表。
        #print(f'INF. ({t2 - t1:.3f}s)')
        #print(f'Done. ({time.time() - t0:.3f}s)')
        return im0, result_txt
